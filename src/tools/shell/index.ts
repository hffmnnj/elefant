...
import { ShellSessionManager } from './session.js';
...
export function createBashTool(deps: BashToolDeps): ToolDefinition<BashParams, string> {
	const sessionManager = new ShellSessionManager();
	sessionManager.setDefaultCwd(deps.projectPath);
		...
		execute: async (params): Promise<Result<string, ElefantError>> => {
			...
			if (params.ephemeral === true) {
				const result = await executeEphemeral(command, {
					cwd: params.cwd ?? deps.projectPath,
					timeoutMs,
				});
...
export const bashTool = createBashTool({ projectPath: process.cwd() });
