...
/**
 * Create a grep tool bound to a specific project directory.
 */
export function createGrepTool(deps: GrepToolDeps): ToolDefinition<GrepParams, string> {
  return {
    name: 'grep',
    ...
    parameters: {
      ...
      path: {
        type: 'string',
        description: 'Directory to search (default: project directory)',
        required: false,
      },
      include: {
        type: 'string',
    ...
    execute: async (params): Promise<Result<string, ElefantError>> => {
      const {
        pattern,
        path: searchPath = deps.projectPath,
        include,
        exclude,
        maxResults = 1000,
      } = params;
...
/**
 * Static grep tool — defaults to `process.cwd()` when no path is given.
 * Used by the static registry; per-run registries should use `createGrepTool`.
 */
export const grepTool: ToolDefinition<GrepParams, string> = createGrepTool({ projectPath: process.cwd() });
